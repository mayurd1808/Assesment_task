...
def generate_with_openai(message):
    ...
    data = post_json("https://api.openai.com/v1/chat/completions", payload, {"Authorization": f"Bearer {api_key}"})
...
def generate_ai_response(message):
    ...
    try:
        if provider == "gemini" and get_setting("GEMINI_API_KEY"):
            return generate_with_gemini(message) or fallback_response(message)
        if get_setting("OPENAI_API_KEY"):
            return generate_with_openai(message) or fallback_response(message)
    except (URLError, TimeoutError, OSError, KeyError, json.JSONDecodeError) as error:
        st.toast(f"AI provider unavailable. Using demo reply. {error}")
